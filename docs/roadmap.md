...
- **2026-05-15** — 客户回复聊天（PRD §3.2）：MessageRecord 加 `direction` 列（outbound / inbound，Alembic revision `c2aa8796a530` + `server_default='outbound'` 兼容老库）；`POST /api/customers/{id}/messages` 走指定 TG 账号发送，落 sent/failed 行 + 写 audit；`GET /api/customers/{id}/messages` 按 created_at 升序返完整双向会话；listen_worker 收到回复时除了沿用"绑到最近 outbound 行"逻辑还**额外插**一条 inbound 行，让聊天历史成立。前端 Replies.vue 重写成微信式：左侧客户列表（按最近活动排序、未读小红点）+ 右侧聊天泡泡（自己绿色右靠，客户白色左靠）+ Ctrl+Enter 发送，WebSocket 实时刷新。242 测试通过（新增 5 个 chat endpoint TDD 覆盖）。
- **2026-05-15** — TG 账号健康字段对租户隐藏：`tenant_scope.sanitize_account_for_tenant()` + `TG_HEALTH_FIELDS={status,last_error,last_login_at,enabled,avatar_status}`。`/api/accounts` 列表对 merchant / business_agent 剥这些字段（admin 不变）；dashboard 的 `accounts` 区段对租户改返 `{total, available}` 二选项（available = active+imported & enabled），不暴露 active/error/imported 拆分，避免后台补号或暂时无号时被察觉；`/api/statistics/accounts` per-account 行也走 sanitizer。6 个新测试（248 测试）。
- **2026-05-15** — 聊天页面消息翻译：`backend/app/services/translator.py` 包装 Google 免费 web 端点（`translate.googleapis.com/translate_a/single`），无需 key；`POST /api/translate {text,target?}` 返回 `{translated_text, source_lang, target_lang}`，失败 502。Replies.vue 每条 inbound 气泡下加「翻译」链接，点了把中文塞中文塞在原文下方（虚线分隔 + "译"小标签），可隐藏；译文缓存在 `m._translated`，重开会话保留。8 个新测试（256 测试）。
- **2026-05-15** — 话术 + 发送前翻译 + 翻译面板 + 客服简化工作台：（A）QuickReply 模型 + `/api/quick-replies` CRUD（personal/public 双模，admin-only 创建公共，owner-only 改私人）；Customer 加 `last_source_lang` 列，翻译 inbound 时缓存语言；`POST /api/translate` 支持 `target='auto'+customer_id` 把中文翻成客户语言。前端聊天框加「话术」抽屉 + 「翻译面板」抽屉 + 「发送前翻译成客户语言」复选框。（B）新增 `ConsoleLayout.vue` 客服简化工作台 + `/console` 路由 + `landingRouteFor(auth)` 角色感知跳转：`role=agent` 登录后直接进 `/console` 不再看到管理后台；admin 顶栏菜单加「客服工作台」入口；Login.vue 同步登录后跳转逻辑。263 测试通过（新增 7 个 quick reply CRUD 覆盖）。
- **2026-05-15** — 双语气泡（参考截图 UX）：MessageRecord 加 `translation` 列（Alembic 49a1aa95b33a），inbound 存中文译文 / outbound 存原文中文；`POST /messages auto_translate=true` 服务端翻译并存原+译两份；新 `POST /customers/{id}/messages/{msg_id}/translate` 端点懒翻译 inbound 并写 DB 缓存（二次调用走缓存零开销）。Replies.vue 气泡重写：原文+译文**同一气泡内**渲染（参考微信式双语气泡），译文淡灰色，可一键收起；打开会话时自动批量翻译未缓存的 inbound（默认开），可在工具栏关闭；outbound 发送时若勾"发送前翻译成客户语言"，bubble 同时显示发出去的英文和原始中文。265 测试通过（新增 2 个 chat endpoint TDD：auto_translate 存双份 + 缓存端点幂等）。
