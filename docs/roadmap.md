...
- **2026-05-15** — 聊天页面消息翻译：`backend/app/services/translator.py` 包装 Google 免费 web 端点（`translate.googleapis.com/translate_a/single`），无需 key；`POST /api/translate {text,target?}` 返回 `{translated_text, source_lang, target_lang}`，失败 502。Replies.vue 每条 inbound 气泡下加「翻译」链接，点了把中文塞在原文下方（虚线分隔 + "译"小标签），可隐藏；译文缓存在 `m._translated`，重开会话保留。8 个新测试（256 测试）。
